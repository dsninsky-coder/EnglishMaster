...
### v0.7（当前）
- **人工复议（新功能）**：学生答错但自认正确时，可花 2 金币申请人工复议，题目推送管理员端由人工判对错；若该步存在待审复议，`step_finish` 时**暂扣**本步通关/完美奖励，待裁决后补发或永久扣留。
...
- **管理员「人工复议」独立栏目**：左侧新增「⚖️ 人工复议」Tab，登录即刷新待办角标（如 `(3)`）；支持待处理/全部切换与一键通过/驳回（附批注）。
